...
1. Work/commit exclusively on the branch given to you.

2. You are allowed to create additional commits on the given branch.

3. Do not merge into `/Branch/Develop/Delphi`. Await Coordinator integration.
...
2. *.go files use import statements pointing to a folder instead of files. Delphi works differently, in delphi the uses clausule must point/mention filenames. So to correctly
   convert the *.go import statements find the correct *.pas units and put those on the uses clausule instead. Consult the file mapping files for the correct delphi fully qualified dotted name.
   
## Special Directives:

1. Do not perform any testing, just convert the specified *.go files to *.pas files or *.dpr files.
