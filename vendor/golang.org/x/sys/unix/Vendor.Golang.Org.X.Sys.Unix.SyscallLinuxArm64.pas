{$MODE DELPHIUNICODE}
unit Vendor.Golang.Org.X.Sys.Unix.SyscallLinuxArm64;
...
function Getrlimit( ParaResource : Integer; ParaRlim : PZRlimit ) : Integer;
begin
	Result := prlimit( 0, ParaResource, nil, ParaRlim );
	if Result = ENOSYS then
	begin
		Result := getrlimit( ParaResource, ParaRlim );
	end;
end;

function Setrlimit( ParaResource : Integer; ParaRlim : PZRlimit ) : Integer;
begin
	Result := prlimit( 0, ParaResource, ParaRlim, nil );
	if Result = ENOSYS then
	begin
		Result := setrlimit( ParaResource, ParaRlim );
	end;
end;
