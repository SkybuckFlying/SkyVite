...
const
  ...
  Magic = #$57#$FB#$80#$8B#$24#$75#$47#$DB;

  BlockTypeNoCompression = 0;
  BlockTypeSnappyCompression = 1;

  FilterBaseLg = 11;
  FilterBase = 1 shl FilterBaseLg;
...
function DecodeBlockHandle(const Src: TBytes; out BlockHandle: TBlockHandle): Integer;
function EncodeBlockHandle(var Dst: TBytes; const ABlockHandle: TBlockHandle): Integer;
...
var
  N, M: Integer;
begin
  N := TZip.ReadUvarint(Src, 0, BlockHandle.Offset);
  M := TZip.ReadUvarint(Src, N, BlockHandle.Length);
  if (N = 0) or (M = 0) then
    Result := 0
  else
    Result := N + M;
...
begin
  SetLength(Temp, 10);
  N := TZip.WriteUvarint(Temp, 0, ABlockHandle.Offset);
  M := TZip.WriteUvarint(Temp, N, ABlockHandle.Length);
  Result := N + M;
  SetLength(Dst, Result);
  System.Move(Temp[0], Dst[0], Result);
end;
