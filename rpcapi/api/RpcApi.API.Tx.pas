...
uses
  ...
  Vite, Common.Types, Common.BigInt, Ledger, Ledger.Consensus, Ledger.Generator,
  Net, Vm.Contracts.Dex, Wallet, RpcApi.Api.LedgerModel, Vm.Quota;

type
  TSendTxWithPrivateKeyParam = record
    SelfAddr: PAddress;
    ToAddr: PAddress;
  ...
  TCalcPoWDifficultyParam = record
    SelfAddr: TAddress;
    PrevHash: THash;
    BlockType: Byte;
    ToAddr: PAddress;
    Data: TBytes;
    UseStakeQuota: Boolean;
    Multiple: Word;
  end;

  TCalcPoWDifficultyResult = record
    Quota: UInt64;
  ...
  TCalcQuotaRequiredParam = record
    SelfAddr: TAddress;
    BlockType: Byte;
    ToAddr: PAddress;
    Data: TBytes;
  ...
  TCalcQuotaRequiredResult = record
    QuotaRequired: string;
  ...
  public
    ...
    procedure SendRawTx(const Block: TAccountBlock);
    function SendTxWithPrivateKey(const Param: TSendTxWithPrivateKeyParam): TAccountBlock;
    // Deprecated
    function CalcPoWDifficulty(const Param: TCalcPoWDifficultyParam): TCalcPoWDifficultyResult;
    function CalcQuotaRequired(const Param: TCalcQuotaRequiredParam): TCalcQuotaRequiredResult;
  end;

implementation

uses System.NetEncoding, System.Math, System.Threading, System.DateUtils;
...
var
  ...
  D: TBigInteger;
begin
  LatestBlock := C.GetLatestAccountBlock(Param.SelfAddr);
  if ((LatestBlock = nil) and (not Param.PrevHash.IsZero)) or
     ((LatestBlock <> nil) and (LatestBlock.Hash <> Param.PrevHash)) then
    raise Exception.Create('Chain forked');

  Block.BlockType := Param.BlockType;
  Block.AccountAddress := Param.SelfAddr;
  Block.PrevHash := Param.PrevHash;
  Block.Data := Param.Data;
  if Param.ToAddr <> nil then
    Block.ToAddress := Param.ToAddr^
  else if Param.BlockType = TBlockType.SendCall then
    raise Exception.Create('toAddress is nil');

  Sb := C.GetLatestSnapshotBlock;
  Db := TVmDb.Create(C, @Param.SelfAddr, @Sb.Hash, @Param.PrevHash);
  QuotaRequired := TVM.GasRequiredForBlock(Db, @Block, TUtil.QuotaTableByHeight(Sb.Height), Sb.Height);

  TQuota.CalcQc(Db, Sb.Height, Qc, IsCongestion);

  if Param.UseStakeQuota then
  begin
    StakeAmount := C.GetStakeBeneficialAmount(Param.SelfAddr);
    Q := TQuota.GetQuota(Db, Param.SelfAddr, StakeAmount, Sb.Height);
    if Q.Current >= QuotaRequired then
    begin
      Result.QuotaRequired := IntToStr(QuotaRequired);
      Result.Difficulty := '';
      Result.UtRequired := FloatToStr(QuotaRequired / TQuota.QuotaPerUt);
      Result.Qc := @Qc.ToString;
      Result.IsCongestion := IsCongestion;
      Exit;
    end;
  end;

  if not TQuota.CanPoW(Db, Block.AccountAddress) then
    raise Exception.Create('CalcPoWTwice');

  D := TQuota.CalcPoWDifficulty(Db, QuotaRequired, Q, Sb.Height);
  if IsCongestion and (Param.Multiple > 10) then
  begin
    D := D * TBigInteger.Create(10);
    D := D div TBigInteger.Create(Param.Multiple);
  end;

  Result.QuotaRequired := IntToStr(QuotaRequired);
  Result.Difficulty := D.ToString;
  Result.UtRequired := FloatToStr(QuotaRequired / TQuota.QuotaPerUt);
  Result.Qc := @Qc.ToString;
  Result.IsCongestion := IsCongestion;
end;
...
begin
  ...
  Sb := C.GetLatestSnapshotBlock;
  Db := TVmDb.Create(C, @Param.SelfAddr, @Sb.Hash, @PrevHash);
  QuotaRequired := TVM.GasRequiredForBlock(Db, @Block, TUtil.QuotaTableByHeight(Sb.Height), Sb.Height);

  Result.QuotaRequired := IntToStr(QuotaRequired);
  Result.TxNumRequired := FloatToStr(QuotaRequired / TQuota.QuotaPerUt);
end;

{ TTx }

constructor TTx.Create(AVite: TVite);
begin
  FVite := AVite;
  FAutoTx := False;
  AutoSend;
...
procedure TTx.SendRawTx(const Block: TAccountBlock);
var
  Lb: TAccountBlock;
...
begin
  ...
  if not CheckTokenIdValid(FVite.Chain, @Lb.TokenId) then
    raise Exception.Create('Invalid token id');
  LatestSb := FVite.Chain.GetLatestSnapshotBlock;
...
function TTx.SendTxWithPrivateKey(const Param: TSendTxWithPrivateKeyParam): TAccountBlock;
var
  Account: TAccount;
  D, Amount: TBigInteger;
  BlockType: Byte;
  Msg: TIncomingMessage;
  AddrState: PAddressState;
  G: TGenerator;
  Result: TGenerateResult;
begin
  if Param.Amount = nil then
    raise Exception.Create('amount is nil');
  if Param.SelfAddr = nil then
    raise Exception.Create('selfAddr is nil');
  if (Param.ToAddr = nil) and (Param.BlockType <> TBlockType.SendCreate) then
    raise Exception.Create('toAddr is nil');
  if (Param.ToAddr <> nil) and (not CheckTxToAddressAvailable(Param.ToAddr^)) then
    raise Exception.Create('ToAddress is invalid');
  if (Param.ToAddr <> nil) and (Param.ToAddr^ = TAddress.DexFund) and (not TDex.VerifyNewOrderPriceForRpc(Param.Data)) then
    raise Exception.Create(SInvalidOrderPriceErr);
  if Param.PrivateKey = nil then
    raise Exception.Create('privateKey is nil');

  Account := TAccount.CreateFromHexKey(Param.PrivateKey^);
  if Account.Address <> Param.SelfAddr^ then
    raise Exception.Create('privateKey is error');

  D := nil;
  if Param.Difficulty <> nil then
  begin
    D := TBigInteger.Create;
    D.SetString(Param.Difficulty^, 10);
  end;

  Amount := TBigInteger.Create;
  Amount.SetString(Param.Amount^, 10);

  if not CheckTokenIdValid(FVite.Chain, @Param.TokenTypeId) then
    raise Exception.Create('Invalid token id');

  if Param.BlockType > 0 then
    BlockType := Param.BlockType
  else
    BlockType := TBlockType.SendCall;

  Msg.BlockType := BlockType;
  Msg.AccountAddress := Param.SelfAddr^;
  Msg.ToAddress := Param.ToAddr;
  Msg.TokenId := @Param.TokenTypeId;
  Msg.Amount := Amount;
  Msg.Fee := nil;
  Msg.Data := Param.Data;
  Msg.Difficulty := D;

  AddrState := TGenerator.GetAddressStateForGenerator(FVite.Chain, @Msg.AccountAddress);
  if AddrState = nil then
    raise Exception.Create('failed to get addr state for generator');

  G := TGenerator.Create(FVite.Chain, FVite.Consensus.SBPReader, Msg.AccountAddress, AddrState.LatestSnapshotHash, AddrState.LatestAccountHash);
  Result := G.GenerateWithMessage(@Msg, @Msg.AccountAddress, Account.Sign);
  if Result.Err <> nil then
    raise Result.Err;

  if Result.VMBlock <> nil then
  begin
    FVite.Pool.AddDirectAccountBlock(Msg.AccountAddress, Result.VMBlock);
    Result := LedgerToRpcBlock(FVite.Chain, Result.VMBlock.AccountBlock);
  end
  else
    raise Exception.Create('generator gen an empty block');
end;

function TTx.CalcPoWDifficulty(const Param: TCalcPoWDifficultyParam): TCalcPoWDifficultyResult;
begin
  Result := RpcApi.Api.Tx.CalcPoWDifficulty(FVite.Chain, Param);
end;

function TTx.CalcQuotaRequired(const Param: TCalcQuotaRequiredParam): TCalcQuotaRequiredResult;
begin
  Result := RpcApi.Api.Tx.CalcQuotaRequired(FVite.Chain, Param);
end;

procedure TTx.AutoSend;
const
  InitContractAddr: array[0..99] of string = (
...
var
  ...
  FromAddrs: TArray<TAddress>;
  FromHexPrivKeys: TArray<string>;
  Key: IKey;
  BinKey, PubKey: TBytes;
  Address: PAddress;
  HexKey: string;
  ToAddr: TAddress;
  Amount: string;
...
begin
  if not FAutoTx then
    Exit;
  if FVite.Producer = nil then
    Exit;

  M := 0;
  N := 0;
  Coinbase := FVite.Producer.GetCoinBase;
  Manager := FVite.WalletManager.GetEntropyStoreManager(Coinbase.ToString);

  SetLength(FromAddrs, 5);
  SetLength(FromHexPrivKeys, 5);
  for I := 0 to 4 do
  begin
    Key := Manager.DeriveForIndexPath(I);
    BinKey := Key.PrivateKey;
    PubKey := Key.PublicKey;
    Address := Key.Address;
    WriteLn(Format('%s hex public key:%s', [Address.ToString, THex.Encode(PubKey)]));
    HexKey := THex.Encode(BinKey);
    FromHexPrivKeys[I] := HexKey;
    FromAddrs[I] := Address^;
  end;

  ToAddr := TAddress.Governance;
  Amount := '0';

  Ss := ['/cF/JQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAnMxAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA',
  ...
  SetLength(Datas, Length(Ss));
  for I := 0 to High(Ss) do
  begin
    V := Ss[I];
    Byts := TNetEncoding.Base64.Decode(V);
    Datas[I] := Byts;
  end;

  Num := 0;

  FVite.Consensus.Subscribe(TTypes.SNAPSHOT_GID, 'api-auto-send', nil,
    procedure(E: TConsensusEvent)
    begin
      if Num > 0 then
      begin
        WriteLn(Format('something is loading[return].%s', [DateTimeToStr(Now)]));
        Exit;
      end;
      Inc(Num);
      try
        SnapshotBlock := FVite.Chain.GetLatestSnapshotBlock;
        if SnapshotBlock.Height < 10 then
        begin
          WriteLn('latest height must >= 10.');
          Exit;
        end;
        State := FVite.Net.Status.State;
        if State <> TSyncState.SyncDone then
        begin
          WriteLn(Format('sync state: %s', [State.ToString]));
          Exit;
        end;
        for I := 0 to N - 1 do
        begin
          for K := 0 to High(FromAddrs) do
          begin
            Addr := FromAddrs[K];
            HexKey := FromHexPrivKeys[K];
            Block := SendTxWithPrivateKey(TSendTxWithPrivateKeyParam.Create(@Addr, @ToAddr, TViteTokenId, @HexKey, @Amount, Datas[Random(Length(Datas))], nil, nil, 2));
            WriteLn(Format('send block:%s,%s,%s', [Block.AccountAddress.ToString, Block.Height, Block.Hash.ToString]));
          end;
        end;
        for I := 0 to M - 1 do
        begin
          ...
          begin
            Addr := FromAddrs[K];
            HexKey := FromHexPrivKeys[K];
            MToAddr := TAddress.FromHex(InitContractAddr[Random(Length(InitContractAddr))]);
            Block := SendTxWithPrivateKey(TSendTxWithPrivateKeyParam.Create(@Addr, @MToAddr, TViteTokenId, @HexKey, @Amount, nil, nil, nil, 2));
            WriteLn(Format('send block:%s,%s,%s', [Block.AccountAddress.ToString, Block.Height, Block.Hash.ToString]));
          end;
        end;
      finally
