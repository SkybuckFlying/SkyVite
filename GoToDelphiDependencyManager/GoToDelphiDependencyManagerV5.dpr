                               program GoToDelphiDependencyManagerV5;
...
begin
  // --- FIX: Skip if the Pascal file doesn't exist yet ---
  ...
  try
    try
      ...
      on E: Exception do begin
        WriteLn('Error loading ' + ADelphiFile + ': ' + E.Message);
        Exit;
      end;
    end;
...
begin
  ...
  WriteLn('--- Summary ---');
  WriteLn(Format('Modified Pas Files:  %d', [FModifiedCount]));
  WriteLn(Format('Skipped (Not converted yet): %d', [FSkippedPasFiles]));
  WriteLn(Format('Missing Go Sources:  %d', [FMissingGoFiles]));
end;
