unit Vendor.Github.Com.Tklauser.GoSysconf.SysconfFreebsd;
...
begin
  case name of
    ...
    SC_CLK_TCK:
      Result := 100; // _CLK_TCK
    SC_DELAYTIMER_MAX:
      Result := sysctl32('p1003_1b.delaytimer_max');
    SC_GETGR_R_SIZE_MAX, SC_GETPW_R_SIZE_MAX:
      Result := -1;
    SC_IOV_MAX:
      Result := sysctl32('kern.iov_max');
    SC_MQ_OPEN_MAX:
      Result := yesno(sysctl32('p1003_1b.mq_open_max'));
    SC_MQ_PRIO_MAX:
      Result := 32; // _MQ_PRIO_MAX
    SC_NGROUPS_MAX:
      Result := sysctl32('kern.ngroups');
    SC_OPEN_MAX:
      Result := -1;
    SC_RTSIG_MAX:
      Result := sysctl32('p1003_1b.rtsig_max');
    SC_SEM_NSEMS_MAX:
      Result := -1;
    SC_SEM_VALUE_MAX:
      Result := 32767; // _SEM_VALUE_MAX
    SC_SIGQUEUE_MAX:
      Result := sysctl32('p1003_1b.sigqueue_max');
    SC_STREAM_MAX:
      Result := -1;
